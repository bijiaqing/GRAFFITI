...
real _col_rate_ij (swarm *dev_particle, int idx_old_i, int idx_old_j)
{
    ...
    // constant kernel
    col_rate_ij = LAMBDA_REF*dev_particle[idx_old_j].grain_numr;
    
    // // linear kernel
    // real size_i = dev_particle[idx_old_i].grain_size;
    // real size_j = dev_particle[idx_old_j].grain_size;
    // col_rate_ij = LAMBDA_REF*RHO_DUST*(size_i*size_i*size_i + size_j*size_j*size_j);
...
//     sqr_velocity += (l_azi / rad / sin(col) - sqrt(G*M_STAR / rad / sin(col)))*(l_azi / rad / sin(col) - sqrt(G*M_STAR / rad / sin(col)));
...
void col_rate_calc (swarm *dev_particle, tree *dev_treenode, real *dev_col_rate, boxf *dev_boundbox)
{
    ...
    {
        candidatelist query_result(MAX_QUERY_DIST);
        cukd::cct::knn<candidatelist, tree, tree_traits>(query_result, dev_treenode[idx_tree].cartesian, *dev_boundbox, dev_treenode, NUM_PAR);
        ...
        for(int j = 0; j < KNN_SIZE; j++)
        {
            ...
            if (idx_query != -1) // if the j-th nearest neighbor exists within MAX_QUERY_DIST
            {
        ...
        real par_azi = static_cast<real>(RES_AZI)*   (dev_particle[idx_old_i].position.x - AZI_MIN) /    (AZI_MAX - AZI_MIN);
        real par_rad = static_cast<real>(RES_RAD)*log(dev_particle[idx_old_i].position.y / RAD_MIN) / log(RAD_MAX / RAD_MIN);
        real par_col = static_cast<real>(RES_COL)*   (dev_particle[idx_old_i].position.z - COL_MIN) /    (COL_MAX - COL_MIN);
...
__global__
void col_flag_calc (real *dev_col_rate, real *dev_col_rand, int *dev_col_flag, real *dev_timestep, curandState *dev_rngs_dim)
{
    int idx = threadIdx.x + blockDim.x*blockIdx.x;
    ...
    if (idx >= 0 && idx < NUM_DIM)
    {
        ...
        real rand_collide = curand_uniform_double(&dev_rngs_dim[idx]); // the range is (0,1]
        real real_collide = (*dev_timestep)*dev_col_rate[idx];

        if (real_collide >= rand_collide)
        {
            dev_col_flag[idx] = 1;
            dev_col_rand[idx] = dev_col_rate[idx]*curand_uniform_double(&dev_rngs_dim[idx]);
...
__global__
void par_evol_calc (swarm *dev_particle, tree *dev_treenode, int *dev_col_flag, real *dev_col_rand, real *dev_col_real, 
    boxf *dev_boundbox, curandState *dev_rngs_par)
{
    ...
    if (idx_tree >= 0 && idx_tree < NUM_PAR)
    {
        ...
        if (dev_col_flag[idx_cell] == 1) // if there is going to be a collision in the cell that the particle is in
        {
            real col_rate_i = dev_particle[idx_old_i].col_rate;
            real rand_collide = dev_col_rand[idx_cell];
            real real_collide = atomicAdd(&dev_col_real[idx_cell], col_rate_i); // atomicAdd returns the value before addition

            if (real_collide < rand_collide && real_collide + col_rate_i >= rand_collide) // if this particle is going to collide
            {
                candidatelist query_result(MAX_QUERY_DIST);
                cukd::cct::knn<candidatelist, tree, tree_traits>(query_result, dev_treenode[idx_tree].cartesian, *dev_boundbox, dev_treenode, NUM_PAR);
                ...
                real rand_collide_ij = col_rate_i*curand_uniform_double(&dev_rngs_par[idx_old_i]);
                ...
                while (real_collide_ij < rand_collide_ij && j < KNN_SIZE)
                {
                    idx_query = query_result.returnIndex(j);
                    ...
                    {
                        idx_old_j = dev_treenode[idx_query].index_old;
                        real_collide_ij += _col_rate_ij(dev_particle, idx_old_i, idx_old_j);
                ...
                if (rel_velocity <= V_FRAG)
                {
                    ...
                    dev_particle[idx_old_i].grain_size  = size_k;
                    dev_particle[idx_old_i].grain_numr *= (size_i/size_k)*(size_i/size_k)*(size_i/size_k);
                }
                else
                {
                    ...
                    real size_j = dev_particle[idx_old_j].grain_size;
                    real size_k = cbrt(size_i*size_i*size_i + size_j*size_j*size_j);
                    real rand   = curand_uniform_double(&dev_rngs_par[idx_old_i]);  // distribution in (0, 1]
...
//     size_1 = dev_particle[idx_1].grain_size;
//     mass_1 = dev_particle[idx_1].grain_numr*size_1*size_1*size_1;
...
//     size_2 = dev_particle[idx_2].grain_size;
//     mass_2 = dev_particle[idx_2].grain_numr*size_2*size_2*size_2;
...
//         cukd::cct::knn <candidatelist, tree, tree_traits> (query_result, dev_treenode[idx].cartesian, *dev_boundbox, dev_treenode, NUM_PAR);
