...
// =========================================================================================================================
// Relative velocity helpers - optimized to take pre-calculated values
...
real _get_ReInvSqrt (real polar_R, real alpha)
{
    ...
    Re = RE_0*(alpha / _get_alpha(R_0, _get_hgas(R_0)))*(sigma / SIGMA_0);
...
real _get_vrel_t (real polar_R, real St_i, real St_j, real h_gas)
{
    ...
    if (vrel_sq < 0.0)
    {
        assert(false, "ERROR: negative vrel_sq in _get_vrel_t\n");
    }
